class <%=class_name %> < ActiveRecord::Base
  ...
  def make_activation_code
    self.activation_code = <%=class_name %>.friendly_token
  end
  ...
  def make_password_reset_token
    self.password_reset_token = <%=class_name %>.friendly_token
  end
  ...
  def reset_password(params={})
    ...
    self.password = params[:<%=model_name %>][:password]
    self.password_confirmation = params[:<%=model_name %>][:password_confirmation]
    self.save
  end
  ...
  def self.authenticate(username, pass)
    <%=model_name %> = <%=class_name %>.find_by_username(username)
    if <%=model_name %>
      <%=model_name %>.authenticate(pass)
    end
  end
